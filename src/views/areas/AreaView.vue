...
const getArea = async function () {
  ...
  try {
    const { data, error } = await supabase.rpc('search_areas', {
      search_name: route.params.name.split('-')[0]
    })
    // from('areas').select('*').eq('id', route.params.name)
    ...
    area.value = data[0]
    otherAreas.value = data.splice(1) // other areas with the same
  } catch (err) {
...
<template>
  ...
  >
          ...
          <div class="flex justify-between items-center">
            <div class="md:w-[60%]">
              <h1 class="text-[24px] capitalize">
                {{ area.area_name }}, {{ area.area_lga }}, {{ area.area_state }} state
              </h1>
              <h2 class="text-[12px] md:text-[16px]">
                <b>“{{ area.review_count }}” Review{{ area.review_count > 1 ? 's' : '' }}</b> (People are raving about the selected location)
              </h2>
            </div>
            <div class="hidden md:flex gap-[16px]">
              <AppButton type="primary" class="uppercase" @click="newReview = true" :disabled="isEmpty(user.username)"
    ...
    >
      ...
      <Reviews clas="md:order-1" ref="reviews" />
    </div>
